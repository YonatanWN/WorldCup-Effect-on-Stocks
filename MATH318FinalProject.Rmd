...
for(rowIndx in 1:samplesize){
  x= getSpyAndGameDataForOneGame(spydata, sampleworldcupgames, rowIndx, 3)
  list = append(list, x)
}
list
```
...
for(i in 1:length(list)){
  plot <- ggplot(as_tibble(list[i])) +  geom_point(mapping = aes(x=date, y=volume), color ="red")
  print(plot)
}
```
...
volumesp4 <- ggplot(game4) + geom_point(mapping = aes(x=date, y=volume), color ="red") + geom_vline(xintercept= sampleworldcupgames$Datetime[4])+ ggtitle("Volume over Time, Sample Game 4")
volumesp4
```

```{r}
game5 <- getSpyAndGameDataForOneGame(spydata, sampleworldcupgames, 5, 3)
volumesp5 <- ggplot(game5) + geom_point(mapping = aes(x=date, y=volume), color ="red") + geom_vline(xintercept= sampleworldcupgames$Datetime[5])+ ggtitle("Volume over Time, Sample Game 5")
volumesp5
```
...
for (game in 1:nrow(cleaned_worldcupmatches)) {
  ...
  highestpriceforgame <- max(price_game_data$average)
  normalize = (highestpriceforgame - overall.min)/(difference)
  time <- price_game_data$"Time_From_Game"
...
optimal_selling.df <- data.frame("Normalized Price" = normalized_price_column, "Time From Game" = time_from_game_column)
```

```{r}
optimal_selling_sp <- ggplot(optimal_selling.df) + geom_point(mapping = aes(x= time_from_game_column, y= normalized_price_column), color ="red") + ggtitle("Optimal Selling Price vs. Time From Game")
optimal_selling_sp
```
