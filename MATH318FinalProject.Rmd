...
for (game in 1:nrow(cleaned_worldcupmatches)) {
  x <- getSpyAndGameDataForOneGame(spydata, cleaned_worldcupmatches, game, 3)
  highestpriceforgame <- max(x$high)
  normalize = (highestpriceforgame - overall.min)/(difference)
  time <- x$"Time_From_Game"
  list = append(list, normalize)
}
list
#highestprice <- max(cleaned_worldcupmatches$average)
```
